...
- `apps/api/src/database/database.service.ts` — wraps a `pg.Pool` from
  ...
  *arguments* go through `$1..$n` placeholders) and
...
- **Gotcha**: mount Better Auth's raw handler via `app.use('/api/auth/*', ...)`
  in `main.ts` *before* Nest's global body parser/`ValidationPipe` touches
...
- **First**, migrate `apps/client` from top-level `app/` to `src/app/`:
  ...
  this *before* adding new pages so it's a clean rename commit. Repeat for `apps/admin`.
